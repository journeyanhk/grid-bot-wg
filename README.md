...
| `HOST` | `127.0.0.1` | 监听地址。VPS 部署设为 `0.0.0.0` |
| `DASHBOARD_USER` / `DASHBOARD_PASS` | 空 | 仪表盘登录账号密码（推荐，VPS 部署必配，见第十六节） |
| `DASHBOARD_SESSION_MS` | `43200000` | 登录会话有效期（毫秒，默认 12 小时） |
| `DASHBOARD_TOKEN` | 空 | 兼容旧版：静态访问令牌（仅当未配置账号密码时生效） |
| `PUBLIC_ORIGIN` | 空 | 允许的页面来源 Origin，逗号分隔。域名/HTTPS 访问时配置 |
...
6. 本程序没有远程服务器、不上传任何数据（配置 AI 助手后会向所选 AI 服务商发送账户状态快照用于分析，属正常数据流），所有状态都在你本机。

### VPS / 公网部署安全配置（必读）

把机器人部署到 VPS 时，控制面板直接暴露在公网，必须做以下配置，否则任何人知道你的服务器地址就能**下单、平仓、撤单**：

1. **设置账号密码**（推荐）：在 `.env` 里配置：
   ```ini
   HOST=0.0.0.0
   DASHBOARD_USER=<你的用户名>
   DASHBOARD_PASS=<openssl rand -base64 16 生成的强密码>
   PUBLIC_ORIGIN=https://你的域名
   ```
   首次打开仪表盘会弹出**账号密码登录框**，登录后获得会话令牌（默认 12 小时有效，可调 `DASHBOARD_SESSION_MS`），所有交易接口均受保护；密码比对为常量时间比较。兼容旧的 `DASHBOARD_TOKEN` 静态令牌模式（二者都配置时以账号密码为准）。
2. **配置 HTTPS 反向代理**（强烈推荐）：用 Nginx / Caddy 把 `80/443` 端口转发到本机 `8080`，并启用 TLS（Let's Encrypt 免费证书）。明文 HTTP 下令牌可能被网络窃听。
3. **防火墙只开放必要端口**：仅放行 `80/443`（或按反代配置放行对应端口），`8080` 不必直接暴露。
4. **令牌泄露即改即生效**：改 `.env` 里的 `DASHBOARD_TOKEN` 并重启，旧令牌立即失效。
5. **其他防护说明**：程序对所有请求做 Origin 校验（非白名单来源一律 403），未配置令牌时仅允许本机来源访问——即使恶意网页把你的服务器域名解析到本机（DNS rebinding）也无法调用交易接口。
6. **日志注意**：SSE 实时流通过 URL 携带令牌，若反代开启访问日志会记录到令牌，属可接受风险（令牌仅用于本机 API，配合 HTTPS 无窃听面）；介意可关闭反代访问日志。
