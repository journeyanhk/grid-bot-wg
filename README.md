...
| `DASHBOARD_TOKEN` | 空 | 仪表盘访问令牌。设置后所有 API 请求必须携带，VPS 部署必配（见第十六节） |
...
1. **设置访问令牌**：在 `.env` 里配置一个强随机串：
   ```ini
   HOST=0.0.0.0
   DASHBOARD_TOKEN=<openssl rand -hex 32 生成的串>
   PUBLIC_ORIGIN=https://你的域名
   ```
   首次打开仪表盘会要求输入令牌（浏览器会话内记住）。所有交易接口均受令牌保护，令牌比对为常量时间比较。
