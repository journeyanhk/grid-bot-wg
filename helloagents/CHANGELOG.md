...
- 文档：how.md 新增「Shadow 第一版参数建议」（中性 / ±3.5% / 15 格 / 0.0015 BTC / 1x / close / 动态网格关闭）、
  **风险口径修正**（17–20 USDC 仅为区间内理论估算、非硬上限；内部日损是阈值非保证）、
  Shadow 验收清单（11 项）与 **sim-write 分阶段切换路径**（只读→单笔远价单→撤单→单笔成交→
  reduce-only 平仓→断线恢复→4–6 格→15 格）
- 验证：用窄区间在 Propr paper 上强制触发越界，close 链路实测通过
  （`价格突破上边界 → 撤单 + 平仓 + 停止 → ✅ 已确认仓位已平`）

### 修复（Shadow 模拟基准与账户口径，2026-09-23）
- Shadow 本地模拟账户改用 **Propr 真实权益**（`marginBalance`）为基准（原来固定 `PAPER_BALANCE=10000`）：
  面板数字与真实账户一致，且 `autoSizePerGrid` 不再按 10000 放大下单量（模拟保真度）
- 详情页「账户状态」新增口径说明（shadow=本地模拟 / sim-write=真实账户）与
  **「Propr 真实账户（只读）」**行（权益/余额/可用/高水位/净仓 + 快照失败标注），避免把模拟余额误读为真实权益
- 部署提示：静态文件按请求从磁盘读取，`git pull` 后刷新浏览器即可见新前端，但**必须重启 Node 进程**
  才会加载新的后端适配器代码（否则 trend 仍返回 paper 合成价 ~100、且字符串 marketId 修复不生效）
