...
### 新增（Review13：A/B 第四影子变体，不拧旋钮）
- `r2faster` 第四变体（高频对照，独立记账/版本 shadow-v1.7.3）：入场阈值 40 / 单次 5M 确认 / ADX 15 / **保留 4H 否决（底线）**；与 r2fast/balanced/strict 并跑 A/B，7 天后读表决定是否提高频率
- 变体级 regime 评估（`regimeAdxMin` 显式开关）：声明后该变体用独立 adxMin 重新评估 regime/评分——使"ADX 15"真实生效（基准信号固定 adxMin=18）；未声明者沿用基准信号，**既有三组样本口径不受影响**
- 跟踪器支持 `confirmChecks`（默认 2；r2faster=1）；日报含第四组

### 说明（Review13 结论背书）
- 低频是 edge：转折期多周期必然不对齐 -> FLAT 是"趋势不明确时空仓"第一原则在执行；不采用拧阈值/去掉双确认/降 ADX/弃 4H 否决等频率旋钮
- 决策对象仍为 r2fast；若 7 天样本不足 30 笔，延长观察期而非调整门槛

### 测试
- regime 增单次确认用例；shadow-quality 增 r2faster 两例（单次确认对照 / 变体级 ADX15）；npm test 全绿
