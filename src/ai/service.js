...
const EXNAMES = { de: 'Decibel', ex: 'Extended', rs: 'RISEx', lr: 'RHC Lighter' };
...
class AiService {
  ...
  async chatControl(message, history = []) {
    ...
    if (j.action.type !== 'none' && !['de', 'ex', 'rs', 'lr'].includes(j.action.exchange)) j.action = { type: 'none' };
