...
async function initExchange(exchange, name, exCfg) {
  try {
    await exchange.init();
    logger.info('server', `[${name}] 连接成功 [${exCfg.mode.toUpperCase()} 模式]`);
  } catch (e) {
    logger.error('server', `[${name}] 初始化失败：${e?.message || e}（目标接口: ${exCfg.apiUrl} 网络: ${exCfg.network}）`);
    const cause = e?.cause || {};
    const code = cause.code || '';
    if (code === 'ENOTFOUND') {
      logger.warn('server', '➤ 域名解析失败：检查网络，或配置代理。');
    } else if (code === 'ECONNREFUSED' && String(cause.address || '').includes('127.0.0.1')) {
      logger.warn('server', '➤ 本机代理端口连不上，检查代理软件是否开启。');
    } else if (code === 'UND_ERR_CONNECT_TIMEOUT' || /timeout/i.test(cause.message || '')) {
      logger.warn('server', '➤ 连接超时，接口被网络拦截，或代理未正确转发。');
    }
    logger.warn('server', `[${name}] 该交易所将以离线模式运行（行情可能使用合成数据）。`);
    // 不退出，让其他交易所继续工作
  }
}
...
async function resumeIfWasRunning(bot, exchange, key) {
  ...
  try {
    logger.info('server', `[恢复] 检测到 ${key.toUpperCase()} 上次为运行状态，正在接管续跑...`);
    await bot.resume(snap);
...
server.listen(cfg.port, cfg.host, () => {
  logger.info('server', `仪表盘已启动: http://${cfg.host === '0.0.0.0' ? 'localhost' : cfg.host}:${cfg.port}（${cfg.de.mode}/${cfg.ex.mode}/${cfg.rs.mode}）`);
  console.log(`\n${'═'.repeat(52)}`);
