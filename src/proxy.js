...
export async function createDispatcher(proxyUrl) {
  ...
  try {
    ...
    logger.error('proxy', '代理库加载失败，请先运行 npm install。错误：' + e.message);
    return null;
  }
}
...
export async function setupProxies(cfg) {
  ...
  if (uniqueProxies.size > 1 && !proxies.global) {
    logger.warn('proxy', '检测到各交易所配置了不同代理，但 GLOBAL_PROXY 未设置。由于 Node.js 全局 fetch 限制，将统一使用第一个有效代理。如需严格隔离代理，请设置 GLOBAL_PROXY 或分开部署各交易所。');
  }
  ...
  try {
    ...
    logger.error('proxy', '设置全局代理失败：' + e.message);
  }
  return { used: null };
}
