...
export class ExtendedExchange extends EventEmitter {
  ...
  async init() {
    ...
    for (let i = 1; i <= 3; i++) {
      ...
      if (i < 3) { logger.info('ex', `市场列表为空/异常，${i}/2 次重试...`); await new Promise((r) => setTimeout(r, 2500 * i)); }
    }
    const list = (data || [])
    ...
    if (!this.markets.size) {
      ...
      try {
        ...
        if (res.status === 403 || /cloudflare|challenge|blocked|denied|captcha/i.test(body)) {
          logger.warn('ex', '疑似代理出口 IP 被交易所风控拦截：请在代理软件里更换节点（换出口 IP），然后点击仪表盘上的「🔌 重连交易所」即可，无需重启程序。');
        }
      } catch (e) {
        logger.error('ex', '诊断请求失败：' + (e?.message || e) + '（网络层面连不通，检查代理）');
      }
    }
  ...
  async _poll() {
    if (this._busy) {
      ...
      logger.warn('ex', '上一轮轮询卡住超过 90 秒，强制解锁继续轮询。');
    }
