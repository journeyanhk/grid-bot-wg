...
export class DecibelExchange extends EventEmitter {
  ...
  async init() {
    ...
    try {
      ...
      throw new Error('未安装 Decibel SDK 依赖，请先在项目目录运行 npm install。底层错误: ' + e.message, { cause: e });
    }
  ...
  async _retry(fn, label, tries = 4) {
    ...
    for (let i = 1; i <= tries; i++) {
      try { return await fn(); } catch (e) {
        ...
        logger.info('de', `读取${label}失败(${e?.cause?.code || e.message})，${i}/${tries - 1} 次重试...`);
  ...
  async _poll() {
    if (this._busy) {
      ...
      logger.warn('de', '上一轮轮询卡住超过 90 秒，强制解锁继续轮询。');
    }
          ...
          if (Number.isFinite(impliedMark) && impliedMark > 0) {
            ...
            if (nowStale) {
              if (!wasStale) {
                ...
                logger.warn('de', `行情源价格(${px ?? 'N/A'})与持仓推算价(${impliedMark.toFixed(2)})偏离过大，疑似索引滞后，风控改用持仓推算价。`);
              }
              px = impliedMark;
              this._prices.set(mId, impliedMark);
            } else if (wasStale) {
              this._pxStale.delete(mId);
              logger.info('de', `行情源已恢复正常(${px})，风控切回直连价格。`);
            }
          }
        }
